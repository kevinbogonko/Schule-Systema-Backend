...
export const getActiveSubjects = async (req, res, next) => {
    ...
    try {
      ...
      const queryText = `SELECT * FROM ${subjectsTable} WHERE status = 1 AND level = $1`;
      const result = await pool.query(queryText, [form]);
...
export const getAllSubjects = async (req, res, next) => {
    ...
    try {
      ...
      const subjectsTable = `subjects`
       ...
       // Query the database with parameterized query
      const queryText = `SELECT * FROM ${subjectsTable} WHERE level = $1`;
      const result = await pool.query(queryText, [form]);
...
export const updateSubjectStatus = async (req, res, next) => {
  ...
  try {
    const { form, updates, isgrouped } = req.body;

    // Validate form
    const parsedForm = parseInt(form);
    if (isNaN(parsedForm)) {
      return next(createError(400, "Invalid level value"));
    }
    ...
    for (const update of updates) {
      const parsedId = parseInt(update.id);
    ...
    // Determine levels to update
    let levelsToUpdate = [parsedForm];
    if (isgrouped === true) {
      if ([-1, 0].includes(parsedForm)) levelsToUpdate = [-1, 0];
      else if ([1, 2, 3].includes(parsedForm)) levelsToUpdate = [1, 2, 3];
      else if ([4, 5, 6].includes(parsedForm)) levelsToUpdate = [4, 5, 6];
      else if ([7, 8, 9].includes(parsedForm)) levelsToUpdate = [7, 8, 9];
      else if ([10, 11, 12].includes(parsedForm)) levelsToUpdate = [10, 11, 12];
    }
    ...
    const query = `
      ...
      WHERE ${subjectTable}.id = data.id
        AND ${subjectTable}.level = ANY($4::int[])
    `;
  ...
  try {
    ...
    const result = await pool.query(
      `SELECT id, name FROM ${subjectTable} 
             WHERE status = 1 AND isselective = 1 AND level = $1`,[sanitizedForm]
    );
...
export const updateSelectiveSubjects = async (req, res, next) => {
  ...
  try {
    ...
    // Validate form
    if (!form || typeof form !== "string" || !/^(-1|[0-9]|1[0-9]|2[0-2])$/.test(form)) {
